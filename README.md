# Agentic Engineer Playbook

**Start here**: [START_HERE.md](START_HERE.md)

> 🆕 **Updated for Claude Code 2.1.0** — January 2026

---

Educational repository for mastering Claude Code.

| Quick Links | |
|-------------|---|
| [Lessons](lessons/) | 10 core modules |
| [Reference](reference/) | Skills, Subagents, Hooks, Settings |
| [Release Notes](learn/claude-code/release-notes/) | Version tracking |
| [Agentic Coding](agentic-coding/) | Advanced TAD training |
| [Exercises](exercises/) | Hands-on practice |
| [Visual Guide](https://miro.com/app/board/uXjVJkcGGBM=/?share_link_id=403387313656) | Presentation & visualisation |

## What's New in 2.1.0

- **Skills hot-reload** — Edit and test skills without restart
- **`context: fork`** — Isolated execution with conversation history
- **Agent-scoped hooks** — Per-agent automation in frontmatter
- **Wildcard permissions** — Pre-approve command families
- **MCP `list_changed`** — Dynamic tool updates

[Full release notes →](learn/claude-code/release-notes/2026-01-07-v2.1.0.md)

---

**Maintainer**: Jakub Strouhal
